...
struct String // a "namespace" not requiring a cpp nor inlining to avoid "unused function" warnings
{
    ...
    static void replaceAll(std::string &str, const std::string &sWhat, const std::string &sWith) {
        ...
        if (sWhat.length())
            while ((foundHere = str.find(sWhat, lookHere)) != std::string::npos) {
    ...
    template <typename T>
    static void split(const std::string &str, const char delimiter, T &result,
                      bool trimmed = true) {
        ...
        if (str.length() && (str.back() == delimiter))
