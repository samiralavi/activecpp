...
public:
    ...
    template <typename... Args>
    static int run(Args &&...args) // run in the calling thread (e.g. main() thread)
    {
    ...
    using Channel = std::function<void(Any &)>;
    ...
    template <typename Any>
    void connect(Channel<Any> receiver = Channel<Any>()) // bind (or unbind) a generic callback
    {
    ...
    void connect(const std::weak_ptr<Him> &receiver) {
        ...
        if (aliveTarget)
            connect(aliveTarget->template getChannel<Any, HighPri>()); // bind another ActorThread
    }
    ...
    void stop(int code = 0) // optional call from ANOTHER thread (suffices deleting the object)
                            // or if created from run()
    {
        if (stop(false))
            exitCode = code; // return code for run() function
    }

    bool exiting() const // mostly to allow active objects running intensive jobs to poll for a
                         // shutdown request
    {
        return !dispatching;
    }
...
protected:
    ActorThread()
        : dispatching(true), externalDispatcher(false), detached(false), exitCode(0),
          mboxPaused(false) {
    }

    virtual ~ActorThread() {
    } // messages pending to be dispatched are discarded
    ...
    /* the active object may use this family of methods to perform the callbacks onto connected
     * clients */

    template <typename Any>
    inline static void publish(Any msg) {
    ...
    template <typename Any>
    void timerStart(const Any &payload, TimerClock::duration lapse, Channel<const Any> event,
    ...
    template <typename Any>
    void timerStart(const Any &payload, TimerClock::duration lapse, // invokes onTimer() methods
                    TimerCycle cycle = TimerCycle::OneShot) {
        Runnable *runnable =
            static_cast<Runnable *>(this); // safe (a dead 'this' will not dispatch timers)
    ...
    template <typename Any>
    void timerReset(const Any &payload) {
    ...
    template <typename Any>
    void timerStop(const Any &payload) {
        auto &allTimersOfThatType = timerEvents<Any>(this);
        auto pTimer = allTimersOfThatType.find(payload);
        if (pTimer != allTimersOfThatType.end()) {
    ...
    struct DispatchRetry // the delivery will be retried later
    {
        ...
        TimerClock::duration
            retryInterval; // will be shortened on every incoming high priority message
    };

    // The following methods are exclusively intended to *interleave* the ActorThread dispatcher
    // with another external dispatcher (e.g. Asio) which will actually be the master dispatcher
    // having the thread control:
    //
    // - onWaitingEvents() must be used to request the external dispatcher to invoke
    // handleActorEvents()
    // - onWaitingTimer() must request the external dispatcher a delayed invocation of
    // handleActorEvents()
    // - onWaitingTimerCancel() must request the external dispatcher to cancel any delayed
    // invocation
    // - onStopping() must request the external dispatcher to end (not required a synchronous
    // completion wait)
    ...
    void onDispatching() {
    } // run the external dispatcher from here (in case it exits, ActorThread resumes again)

    void onWaitingEvents() {
    } // invoked from another threads (new messages coming)
    void onWaitingTimer(
        TimerClock::duration);   // invoked from dispatcher thread (there is only a single timer)
    void onWaitingTimerCancel(); // invoked from dispatcher thread (will come even if the timer
                                 // was not in use)
    void onStopping() {
    } // invoked from another threads (mandatory handling: the object could be about to be
      // deleted)
...
private:
    ...
    public:
        ...
        ActorQueue()
            : head(reinterpret_cast<Item *>(new Aligned<Item>)), // dummy transient placeholder
              tail(head.load(std::memory_order_relaxed)), count(0), lastFront(nullptr),
        ...
        ~ActorQueue() {
            clear();
            ::operator delete(
                head.load(std::memory_order_relaxed)); // also suitable for a dummy instance
        }
        ...
        inline Item *
        front() // returns nullptr if empty (must be always invoked just before pop_front())
        {
        ...
        void pop_front() // also destroys and ultimately deletes the item
        {
            ...
            lastFront->~Item(); // (atomic destructor is trivial) memory deletion is actually
                                // deferred one step behind
    ...
    private:
        std::atomic<Item *>
            head; // the stored objects hold the linked list pointers (single memory allocation)
...
private:
    ...
    template <typename Any>
    struct ActorCallback : public ActorParcel {
        ActorCallback(Channel<Any> &&msg) : message(std::move(msg)) {
        }
        void deliverTo(Runnable *) {
            callback<Any>() = std::move(message);
        }
        Channel<Any> message;
    };
    ...
    template <typename Any>
    struct ActorAlarm : public ActorTimer {
        ActorAlarm(Channel<const Any> &&fn, const Any &p) : event(std::move(fn)), payload(p) {
        }
        void deliverTo(Runnable *instance) {
            this->shoot = true;
            if (event)
                event(payload); // the invoked function could "touch" (shoot -> false) this very
                                // same timer
            if (this->shoot) {
    ...
    void timerReschedule(std::shared_ptr<ActorTimer> &&timer, bool incremental) {
        timers.erase(
            timer); // resetting will require a position change in the set nearly 100% of times
        timer->reset(incremental);
        timers.insert(std::move(timer)); // emplaced in the new position
    }

    template <typename Any>
    static std::map<Any, std::weak_ptr<ActorAlarm<Any>>> &timerEvents(ActorThread *caller) {
    ...
    static void actorThreadRecycler(Runnable *runnable) {
        if (runnable->stop(true))
            delete runnable; // deletion is deferred when not possible (detaching the thread)
    }
    ...
    bool stop(bool forced) try // return false if couldn't be properly stop
    {
        ...
        } else // normal stop invoked from another thread (or if started from run())
        {
            ...
            static_cast<Runnable *>(this)->onStopping();
            if (!fromCreate)
                return true; // queues don't require and can't be cleared (potentially inside
                             // onMessage())
            runner.join();
            timers.clear();
...
protected:
    ...
    void post(Any &&msg) // runs on the calling thread
    {
        ...
        if (HighPri)
            mboxPaused = false;
        if (!isIdle)
            return; // if the consumer has pending messages (e.g. under high load) this method
                    // returns here
        std::lock_guard<std::mutex> lock(
            mtx); // under high load is only acquired in eventsLoop() (no effective lock)
...
private:
    ...
    void retryMbox(const DispatchRetry &) {
        mboxPaused = false;
    }
        ...
        while (dispatching && mustDispatch) {
            ...
            if (!mboxPaused && (hasHigh || hasNorm)) // consume the messages queue
            {
                ...
                try {
                    ...
                    auto event = Channel<const DispatchRetry>([this](const DispatchRetry &dr) {
                        retryMbox(dr);
                    });
            ...
            if (firstTimer == timers.cend()) {
                std::unique_lock<std::mutex> ulock(
                    mtx); // lock required *here* to overcome the sleeping barber problem
                ...
                } else                               // the other timers are scheduled even further
                {
                    ...
                    if (dispatching && mboxHighPri.empty() && (mboxNormPri.empty() || mboxPaused)) {
                        ...
                        messageWaiter.wait_until(
                            ulock, wakeup); // wait until first timer or for incoming messages
                    }
                }
            }
        }
    ...
    struct ActorPointedKeyComparator {
