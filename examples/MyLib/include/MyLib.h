...
struct Billing {
  std::atomic<int>
      count; // shared message (same raw pointer accessed by two threads)
};
...
public:
  ...
  void
  basicSubscriptions(const Client &client) // basic data reception by clients
  {
    ...
    connect<const std::shared_ptr<Billing>>(
        client); // const only to fulfil the timers signature for the weird
                 // example
  }
};
