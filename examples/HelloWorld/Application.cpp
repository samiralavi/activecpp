...
int main(int argc, char **argv) {
    return Application::run(
        argc, argv); // blocking call (Application::weak_from_this() will not expire until stop())
}
...
void Application::onStart() {
    ...
    world = World::create(
        weak_from_this().lock()); // lock() returns a non empty reference (see comment in main())
    world->send(printer);
...
void Application::onMessage(Picture &msg) {
    printer->send(
        LINE("<application> is responded: " << msg.width << "x" << msg.height << " picture"));
}
...
void Application::onTimer(const int &) {
    stop(123); // valid call (self-terminate request) from threads started by run(); with optional
               // exit code
}
