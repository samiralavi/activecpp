...
void Application::onStart() {
  ...
  library->connect<std::shared_ptr<ReplyB>>(
      weak_from_this()); // (alternative syntax)

  library->send(WantPrinter{});
}

template <> void Application::onMessage(Printer::ptr &msg) { printer = msg; }

template <> void Application::onMessage(std::unique_ptr<Info> &msg) {
  printer->send(LINE("<MyApp> received " << msg->data));

  // Programmers not very seasoned managing the objects lifecycle may be
  // concerned about the risk of 'library' being potentially deleted at the
  // moment they need to invoke its send() method. In the following example, a
  // safeLibrary() functor is used instead, which wouldn't crash even in such
  // situation:

  if (msg->data.find("fast") != std::string::npos)
    safeLibrary(std::make_shared<RequestA>(
        "RequestA")); // equivalent to library->send()
  else
