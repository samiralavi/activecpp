...
template <>
void MyLib::onMessage(WantPrinter &) {
    ...
    auto billingAddress = callback<const std::shared_ptr<Billing>>(); // billingAddress(bill) is
...
template <>
void MyLib::onMessage(std::shared_ptr<RequestA> &msg) {
...
template <>
void MyLib::onMessage(std::shared_ptr<RequestB> &msg) {
    printer->send(LINE("<MyLib> received " << msg->data));
...
template <>
void MyLib::onTimer(const std::string &whatEvent) {
    publish(std::unique_ptr<Info>(new Info{whatEvent}));
}

template <>
void MyLib::onTimer(const char &acter) {
    printer->send(LINE("<MyLib> beat " << acter));
}

template <>
void MyLib::onTimer(const LibraryIsTired &seriously) {
    publish(seriously);
}
