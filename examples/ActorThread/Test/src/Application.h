...
struct BreedImplode {
    std::shared_ptr<class Task> child;
    int implosions;
};
...
class Task : public ActorThread<Task> {
    ...
    Task(std::shared_ptr<class Application> parent)
        : app(parent), gen(std::random_device{}()), rnd(0, 9), syncTestCompleted(false),
          mixedTestCompleted(false), mixedTestPaused(false), fstats{0, 0, 0, 0}, implosions(0) {
    }

    Task(Task::ptr parent) : ancestor(parent), implosions(0) {
    } // for breeding test
